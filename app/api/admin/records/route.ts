...
type AdminRecord={id:number;name:string;handle:string;initial:string;color:string;debut:string;last:string;category:string;note:string;bio:string;tags:string[];published?:boolean|number};
...
export async function PUT(request:Request){
  ...
  await db().prepare(`UPDATE records SET name=?,handle=?,initial=?,color=?,debut=?,last_activity=?,category=?,note=?,bio=?,tags=?,published=?,updated_at=CURRENT_TIMESTAMP WHERE id=?`).bind(p.name,p.handle,p.initial,p.color,p.debut,p.last,p.category,p.note,p.bio,JSON.stringify(p.tags||[]),p.published===false||p.published===0?0:1,p.id).run();
