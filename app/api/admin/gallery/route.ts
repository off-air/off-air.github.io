...
export async function POST(request:Request){
  ...
  if(thumbnails.length!==files.length||thumbnails.some(file=>file.type!=='image/webp'||file.size>256*1024))return Response.json({error:'썸네일 생성 결과를 확인해주세요.'},{status:400});
