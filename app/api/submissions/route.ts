...
export async function POST(request: Request) {
  try {
    ...
    if (!isText(body.type, 40) || !isText(body.name, 100) || !isText(body.channelUrl, 2048) || !isText(body.message, 4000)) return Response.json({ error: "입력 내용을 확인해주세요." }, { status: 400 });
    if (body.sourceUrl != null && body.sourceUrl !== "" && (!isText(body.sourceUrl, 2048) || !validUrl(body.sourceUrl as string))) return Response.json({ error: "출처 주소를 확인해주세요." }, { status: 400 });
    if (!validUrl(body.channelUrl as string)) return Response.json({ error: "활동 채널 주소를 확인해주세요." }, { status: 400 });
